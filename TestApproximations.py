...
def doCalculations(claims_distribution_name, claims_params, severity_distribution_name, severity_params):
    ...
    if severity_distribution_name == "Pareto":
        ...
        s_min = 1 + epsilon
    s_max = s_min + 1000
    num_points = 1001  # Number of points to plot
    s_values = np.linspace(s_min, s_max, num_points)

    # Convert to Python float
    s_values = [round(float(s), 3) for s in s_values]
    ...
    result = panjer_recursion_bounds(a,b,s_values,N,claims_distribution_name,claims_params,severity_distribution_name,severity_params)
